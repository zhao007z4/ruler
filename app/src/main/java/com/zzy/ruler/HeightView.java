...
public class HeightView extends View {
    ...
    private double physicsH=0,physicsW=0;
    private int screenW,screenH=0;
    ...
    public HeightView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        if (attrs != null) {
            ...
            mHighLightColor = attributes.getColor(R.styleable.HeightView_highColor, mHighLightColor);
    ...
    private void init() {
        ...
        mShortLineLength = TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 10, getResources().getDisplayMetrics());
        mMiddleLineLength = TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 15, getResources().getDisplayMetrics());
                ...
                switch (mOrientation) {
                    case VERTICAL: {
                        ...
                        if (distanceY > 0) {
                            ...
                            int minDistance = (int)((mLines * space - (getHeight() >> 1)) + getPaddingBottom());
                    ...
                    case HORIZONTAL: {
                        //往左滚动
                        if (distanceX > 0) {
                            int maxX = (int)((mLines /*+ mStartLineValue*/) * space - (getWidth() >> 1) + getPaddingLeft());
            ...
            case MeasureSpec.UNSPECIFIED: {
                switch (mOrientation) {
                    ...
                    default: {
                        if(screenH!=0)
                        {
                            space = screenH/physicsH;
                            double viewPhysicsH = physicsH*getMeasuredHeight()/screenH;
                        }
                        return (int)(mLines * space);
                    }
                }
    ...
    public void adjustMarker(boolean adjustPosition) {
        ...
        switch (mOrientation) {
            ...
            case HORIZONTAL: {
                ...
                int expectX = (int)(space * mCurrentLineIndex + startX);
    ...
    protected void drawVertical(Canvas canvas) {
        ...
        for (int i = 0; i <= mLines; i++) {
            ...
            switch (i % mOutSideLine) {
                case ZERO:
                    ...
                    float textLeft = left+mLongLineLength+mMarkerSpace+maxTextWidth/2;
                    canvas.drawText(text, textLeft/*left + (maxTextWidth - currentTextWidth) / 2*/, (int)(bottom - i * space), mTextPaint);
            ...
            mLinesArr[i * 4 + 1] = (int)(bottom - i * space);
            mLinesArr[i * 4 + 2] = left /*+ maxTextWidth*/ + lineLength;
            mLinesArr[i * 4 + 3] = (int)(bottom - i * space);
        }
        ...
        canvas.drawLine(left + maxTextWidth,
                (int)(bottom - mCurrentLineIndex * space),
                left + maxTextWidth + (((mCurrentLineIndex) % mOutSideLine == 0) ? mLongLineLength : mShortLineLength),
                (int)(bottom - mCurrentLineIndex * space),
                mHighlightPaint);

        canvas.drawText("mm", getWidth()-getPaddingRight()+maxTextWidth/2, (int)(bottom - mLines * space), mTextPaint);
        canvas.drawText("mm", getWidth()-getPaddingRight()+maxTextWidth/2, bottom, mTextPaint);
    }
    ...
    protected void drawHorizontal(Canvas canvas) {
        ...
        for (int i = 0; i <= mLines; i++) {
            ...
            switch (i % mOutSideLine) {
                case ZERO:
                    String text = String.valueOf(value);
                    canvas.drawText(text, (int)(left + i * space), bottom, mTextPaint);
                    lineLength = mLongLineLength;
                    break;
                default:
                    lineLength = mShortLineLength;
            ...
            /*startX*/
            mLinesArr[i * 4] = (int)(left + i * space);
            /*startY*/
            mLinesArr[i * 4 + 1] = bottom - mTextPaint.getTextSize();
            /*stopX*/
            mLinesArr[i * 4 + 2] = (int)(left + i * space);
            /*stopY*/
...
//        //绘制当前选中的线条
        canvas.drawLine((int)(left + mCurrentLineIndex * space),
                (bottom - mTextPaint.getTextSize()),
                (int)(left + mCurrentLineIndex * space),
    ...
    public void setPhysicSize(double physicsW,double physicsH,int screenW,int screenH)
    {
        this.physicsW = physicsW;
        this.physicsH = physicsH;
        this.screenW = screenW;
        this.screenH = screenH;

        int viewH= getHeight();
        double viewPhysicsH = 0;
        if(viewH!=0)
        {
            viewPhysicsH = physicsH*viewH/screenH;
            space = screenH/physicsH;
            setSpace(space);
        }
    }
    ...
    public void setCurrentLineIndex(int currentLineIndex) {
        int distance = (int)(currentLineIndex * space);
